...
// icon from config) plus BuildChannelProgrammes output for each.
func BuildGuideXML(snap *lineup.Snapshot, idx *Index, now time.Time) ([]byte, error) {
	doc := xmltv.Document{}
	...
	for _, ch := range snap.Channels {
		...
		if ch.Logo != "" {
			xc.Icon = &xmltv.Icon{Src: ch.Logo}
		}
